...
int main()
{
	...
	FILE *fram = fopen(R"(C:\Users\lxy98\Documents\MyGitProjects\lr_rsicv_cpu\test\testcase\example\example.bin)", "rb");
	...
	comm.always_read([&](const std::vector<unsigned char> &data)
	{
		if (data.size() == 5 && data[4] == 0)
		{
			...
			unsigned int addr = (data[0] | data[1] << 8 | data[2] << 16 | data[3] << 24) - 4;
			if (addr >= MEMORY_SIZE)
			...
			else
			{
				...
				//printf("Read Addr 0x%08x: %02x%02x%02x%02x\n", addr, (int)memory[addr], (int)memory[addr + 1], (int)memory[addr + 2], memory[addr + 3]);
				comm.write({ memory[addr], memory[addr + 1], memory[addr + 2], memory[addr + 3] });
				//comm.write({ memory[addr + 3], memory[addr + 2], memory[addr + 1], memory[addr + 0] });
			}
		}
		else if (data.size() == 9)
		{
			//unsigned int addr = data[4] | data[5] << 8 | data[6] << 16 | data[7] << 24;
			unsigned int addr = (data[4] | data[5] << 8 | data[6] << 16 | data[7] << 24) - 4;
			...
			else
			{
				...
				if (data[8] & 0b0010)
					memory[addr + 2] = data[1];
				if (data[8] & 0b0100)
					memory[addr + 1] = data[2];
				if (data[8] & 0b1000)
					memory[addr + 0] = data[3];*/
			}
		}
	});
	return 0;
